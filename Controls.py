...
def update_bullets(screen,stats,sc,inos,bullets):
    #Уничтожаем пришельцев
    bullets.update()
    for bullet in bullets.copy():
        if bullet.rect.bottom<=0:
            bullets.remove(bullet)
    collosions = pygame.sprite.groupcollide(bullets,inos,True,True)

    if collosions:
        for inos in collosions.values():
